...
# Keystone installasi dan konfigurasi
...
# Konfigurasi Keystone untuk database Mysql
...
#################### Membuat aturan, tenants dan user #################### 
...
# Create user
PASSWORD=openstack
keystone user-create --name coba --tenant_id $TENANT_ID --pass $PASSWORD --email coba@localhost --enabled true

TENANT_ID=$(keystone tenant-list | awk '/\ admin\ / {print $2}')

ROLE_ID=$(keystone role-list | awk '/\ Member\ / {print $2}')

USER_ID=$(keystone user-list | awk '/\ coba\ / {print $2}')

# menambahkan aturan member coba di admin
keystone user-role-add --user $USER_ID --role $ROLE_ID --tenant_id $TENANT_ID
...
sudo sed -i 's#^auth_host.*#auth_host = server1#' /etc/glance/glance-registry.conf
...
sudo sed -i 's#^auth_host.*#auth_host = server1#' /etc/glance/glance-api.conf
...
sudo mkdir /home/openstack/images

wget https://www.dropbox.com/s/vrmyobubfnyl6ch/cirros-0.3.1-x86_64-disk.img /home/openstack/images
glance image-create --is-public true --disk-format qcow2 --container-format bare --name "Cirros 0.3.1" < /home/openstack/images/cirros-0.3.1-x86_64-disk.img
...
sudo sed -i 's#^auth_host.*#auth_host = server1#' /etc/nova/api-paste.ini
...
sudo sed -i "s/^OPENSTACK_HOST.*/OPENSTACK_HOST = \"server1\"/g" /etc/openstack-dashboard/local_settings.py
